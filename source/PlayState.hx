...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'bghorse':
			var bg:BGSprite = new BGSprite('bg/LONGHORSE/long bg0', 0, 0, 0.9, 0.9);
			bg.scale.set(2, 2);
			add(bg);

			var floor:BGSprite = new BGSprite('bg/LONGHORSE/long bg1', 0, 0, 0.9, 0.9);
			floor.scale.set(2, 2);
			add(floor);

			case 'ccbg':
			var bg:BGSprite = new BGSprite('bg/CARTOONCAT/bg', 0, 0, 0.9, 0.9);
			bg.scale.set(2, 2);
			add(bg);
	
			var floor:BGSprite = new BGSprite('bg/CARTOONCAT/floor', 0, 0, 0.9, 0.9);
			floor.scale.set(2, 2);
			add(floor);

			case 'totlbg':
			bgtotl = new BGSprite('bg/TOTL/bg', 0, 0, 0.9, 0.9);
			bgtotl.scale.set(2, 2);
			add(bgtotl);
		
		    floortotl = new BGSprite('bg/TOTL/floor', 0, -50, 0.9, 0.9);
			floortotl.scale.set(2, 2);
			add(floortotl);

			blackout = new BGSprite('minecraft/TOTL/black', 0, 0, 0.9, 0.9);
			blackout.setGraphicSize(Std.int(blackout.width * 2.5));
		...
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, 0xFF2E2A6B, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Lights Out':
				var timeN:Float = 0;
				var OFF:Float = Std.parseFloat(value1);
				var ON:Float = Std.parseFloat(value2);
				if(Math.isNaN(OFF)) OFF = 0;
				if(Math.isNaN(ON)) ON = 1;
