...
class TitleState extends MusicBeatState
{
	...
	var ngSpr:FlxSprite;
	var bgforest:FlxSprite;
	var yipeeee:FlxSprite;
	...
	function startIntro()
	{
		...
		bgforest = new FlxSprite().loadGraphic(Paths.image('TitleBG'));
		bgforest.scrollFactor.set(0,0);
		bgforest.updateHitbox();
		bgforest.screenCenter();
		bgforest.antialiasing = true;
		add(bgforest);
        ...
        yipeeee = new FlxSprite(0, 0);
		yipeeee.antialiasing = ClientPrefs.globalAntialiasing;
		yipeeee.frames = Paths.getSparrowAtlas('SirenHead_Menu_Assets');	
		yipeeee.animation.addByPrefix('bump', 'Siren Head Menu', 24, true);
		yipeeee.animation.play('bump');
		yipeeee.screenCenter(X);
		yipeeee.y += 200;
		yipeeee.updateHitbox();
		add(yipeeee);
